...
const C_DATA_OUTPUT_PROVENANCE_SOURCE: &str = r#"
#include "dag_ml.h"
...
int main(int argc, char **argv) {
    ...
    if (DAG_ML_GRAPH_SPEC_SCHEMA_VERSION != 1u ||
        DAG_ML_CAMPAIGN_SPEC_SCHEMA_VERSION != 1u ||
    ...
    contract.len = 0;
    contract.capacity = 0;
    status = dagml_campaign_spec_contract_json(&contract, &error);
    if (status != DAG_ML_STATUS_OK) {
        fprintf(stderr, "campaign contract export failed with status %u: %.*s\n",
            status,
            (int)error.len,
            error.ptr ? error.ptr : "");
        if (error.ptr) {
    ...
    if (!contract.ptr ||
        !contains_bytes(contract.ptr, contract.len, "\"schema_version\":1") ||
        !contains_bytes(contract.ptr, contract.len, "campaign_spec.v1.schema.json")) {
        fprintf(stderr, "unexpected campaign spec contract: %.*s\n",
            (int)contract.len,
            contract.ptr ? (char *)contract.ptr : "");
        if (contract.ptr) {
            dagml_owned_bytes_free(contract);
    ...
    contract.len = 0;
    contract.capacity = 0;
    status = dagml_controller_manifest_contract_json(&contract, &error);
    if (status != DAG_ML_STATUS_OK) {
        fprintf(stderr, "controller manifest contract export failed with status %u: %.*s\n",
            status,
            (int)error.len,
            error.ptr ? error.ptr : "");
        if (error.ptr) {
    ...
    if (!contract.ptr ||
        !contains_bytes(contract.ptr, contract.len, "\"schema_version\":1") ||
        !contains_bytes(contract.ptr, contract.len, "controller_manifest.v1.schema.json")) {
        fprintf(stderr, "unexpected controller manifest contract: %.*s\n",
            (int)contract.len,
            contract.ptr ? (char *)contract.ptr : "");
        if (contract.ptr) {
            dagml_owned_bytes_free(contract);
    ...
    if (!contract.ptr ||
        !contains_bytes(contract.ptr, contract.len, "\"schema_version\":1") ||
        !contains_bytes(contract.ptr, contract.len, "\"extra_key\":\"dag_ml_output\"") ||
        !contains_bytes(contract.ptr, contract.len, "data_output_provenance.v1.schema.json")) {
        fprintf(stderr, "unexpected data-output provenance contract: %.*s\n",
            (int)contract.len,
            contract.ptr ? (char *)contract.ptr : "");
        if (contract.ptr) {
            dagml_owned_bytes_free(contract);
    ...
    Buffer provenance = read_file(argv[1]);
    status = dagml_data_output_provenance_validate_json(
        provenance.ptr,
        provenance.len,
        &error
    );
    ...
    if (status != DAG_ML_STATUS_VALIDATION_ERROR || !error.ptr) {
        fprintf(stderr, "invalid provenance did not fail as expected, status=%u\n", status);
        return 1;
    }
...
#[test]
fn c_program_validates_data_output_provenance_against_c_abi() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let workspace = manifest_dir
        .parent()
        .and_then(Path::parent)
    ...
    let dynamic_lib_dir = dynamic_lib
        ...
        .to_path_buf();
    let temp = std::env::temp_dir().join(format!(
        "dag_ml_data_output_provenance_conformance_{}_{}",
        std::process::id(),
        unique_suffix()
    ));
    fs::create_dir_all(&temp).expect("create data-output provenance conformance temp dir");

    let c_path = temp.join("data_output_provenance.c");
    let exe_path = temp.join("data_output_provenance");
    fs::write(&c_path, C_DATA_OUTPUT_PROVENANCE_SOURCE)
        .expect("write data-output provenance C conformance source");
    ...
    assert!(
        compile_output.status.success(),
        "data-output provenance C conformance compile failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&compile_output.stdout),
        String::from_utf8_lossy(&compile_output.stderr)
    );
