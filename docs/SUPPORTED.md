...
| Runtime process adapter protocol | Supported | JSONL frames, describe handshake, timeouts, retries and worker pools are covered by CLI tests. Task execution has no timeout by default so long model fits are allowed; `--process-timeout-ms` opts into a finite limit (0 disables it). It is not an HPO protocol: process controllers cannot attest native optimizer state, trial history, checkpoints or terminalization and therefore refuse tuner-session creation. |
...
`dag-ml` 0.3.27 consumes the sibling `dag-ml-data` 0.2.12 contracts through
