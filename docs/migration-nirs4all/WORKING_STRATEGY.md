...
- `dag-ml` is a **separate, contract-frozen submodule** (0.2.x RC, currently 0.2.7) co-evolving with `dag-ml-data` under the ADR-10 release train. It is wheel-buildable/importable from the built PyO3 wheel; PyPI publishing is a configured release target.
