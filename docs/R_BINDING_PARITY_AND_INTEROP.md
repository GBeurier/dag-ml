...
**État courant du 26 septembre, publication R-universe validée pour ces sources.**
...
donne ce même `RemoteSha` ; son [run R-universe](https://github.com/r-universe/gbeurier/actions/runs/36241951517)
a réussi sur source, Linux, macOS, Windows et WASM. La [PR R #30](https://github.com/GBeurier/nirs4all-r/pull/30)
puis le correctif [#32](https://github.com/GBeurier/nirs4all-r/pull/32)
sont fusionnés sur `main` `b716e00` ; l'[API publique R](https://gbeurier.r-universe.dev/api/packages/nirs4all)
sert 0.4.0.9031 depuis ce `RemoteSha`. Son [run R-universe 36244526111](https://github.com/r-universe/gbeurier/actions/runs/36244526111)
a réussi sur source, Linux, macOS, Windows et WASM. La [PR Python #149](https://github.com/GBeurier/nirs4all/pull/149)
est fusionnée sur `main` `38fa7b4` avec CI verte ; il s'agit de la source
Python, **pas** d'une mise à jour du paquet n4m sur PyPI.

Le tarball R v6 exact du commit `b716e00`, SHA-256
`0528e151674b67af0c2d32e1ac6f4b8207ed9e9905795bc6f552dbe1076d0f93`,
...
Le workbench CRAN local `nirs4all-r-cran-workbench-n4mp-v6-15e89789-b716e00-2026-09-26.tar.gz`
(hors dépôt DAG-ML, SHA-256
`b219ae284626f0bafc6451cdec255586b9b9fdcccf4a39a45701f60f254f3d84`)
assemble 116 sources dont les noms, versions et empreintes ont été vérifiés
après extraction ; ce n'est **ni une soumission ni une acceptation CRAN**.
`n4m` reste une dépendance forte hors des dépôts CRAN/Bioconductor.
...
réentraînement sont intégrées dans la source `main` via #149 ; cela ne
qualifie pas automatiquement un artefact PyPI. La [PR Python #148](https://github.com/GBeurier/nirs4all/pull/148),
...
La [PR Methods #36](https://github.com/GBeurier/nirs4all-methods/pull/36)
prépare séparément l'ABI 2.11 et les paquets R `n4m`/`pls4all` 1.0.21.9006 :
elle reste **draft**, sa CI est en relance et un contrôle local du prochain
produit R est en cours. Elle propose de faire passer les façades R nommées
de **83 à 113** (+8 splitters, +22 augmentations) ; c'est une **route de
couverture structurelle**, pas 30 oracles de parité numérique ni une
publication. Le tableau ABI 2.10 ci-dessous demeure la photographie publique
83 R / 84 JS-WASM.
...
### Inventaire des interfaces n4m (ABI 2.10.0 publique, photographie statique)
...
`RemoteSha=15e89789` et tous ses contrôles source/Linux/macOS/Windows/WASM
sont `OK`. Cela ne convertit pas les artefacts R spécifiques à l'hôte en
états portables.
...
complétée par [#32](https://github.com/GBeurier/nirs4all-r/pull/32) et
publiée depuis `main` `b716e00` (tarball exact contrôlé à 0 erreur,
1 avertissement CRAN incoming, 0 note), ajoute une enveloppe v6 N4MP+N4MM
sur des chaînes linéaires sémantiquement qualifiées (SNV par défaut, MSC,
detrend et SavGol
...
possède des tests bidirectionnels R↔Python et est fusionnée avec CI verte.
Le [run R-universe R 36244526111](https://github.com/r-universe/gbeurier/actions/runs/36244526111)
passe les plateformes publiques après correction de l'oracle d'ensemble
stochastique macOS. Les anciens
...
avec CV/OOF/refit/rejeu DAG-ML, recettes R explicites et intégration
`nirs4all-formats`. La nouvelle enveloppe n4m entraînée v2 transfère aussi
sparse PLS-DA entre R et Python : labels ordonnés, scores N4MM natifs,
prédiction sur holdout et réentraînement dans les deux hôtes, testés sur les
profils simple, SNV et MSC. L'enveloppe v1 PLS reste compatible. Le tarball R
exact 0.4.0.9030 (`de84c008ffbe705653ce07cbf3783363075367be40541fc2e136ba2d7494a419`
SHA-256) a passé le contrôle Linux/R 4.6.0 strict avec tous les `Suggests` :
0 erreur, seul avertissement CRAN-incoming attendu. Cette enveloppe bornée
**n'est pas** une Archive V2/V3 DAG-ML ; elle ne couvre pas le niveau 2 général
ni WASM. L'ABI C DAG-ML sait maintenant exporter un `PortablePredictorPackage`
...
été comparés. Le candidat ajoute aussi `nirs4all_n4m_selector()` pour les 25
sélecteurs du dispatcher Methods : fits natifs, état RDS et OOF/REFIT DAG.
Le nœud commun `n4m.Selector` est qualifié localement R↔Python sur les 25
recettes JSON/YAML : indices classés, projection des matrices hors entraînement
et prédictions PLS, avec graines explicites quand requises. L'enveloppe v4
transfère leur état entraîné avec ces indices et une recette réentraînable :
les 25 passent l'export/import local R et Python ; trois compositions
directe/séquentielle/branche passent aussi R↔Python dans les deux sens.
Cela ne qualifie pas l'exécution WASM réelle.
...
La dernière version publique R-universe vérifiée à ce jalon est 0.4.0.9030 ;
l'accès réseau était alors indisponible. Cette validation
...
GitHub était inaccessible à ce jalon (DNS et authentification),
donc ce candidat n'avait été ni poussé, ni fusionné, ni publié sur R-universe.
...
SPA et les 25 sélecteurs `n4m.Selector` en recette/rejeu. Methods JS lit les
indices `int64` sans réduction de précision et son shim harmonise les défauts
des huit régressions et refuse les modèles aux coefficients/moyennes incomplets.
Les tests JS avec moteur simulé, les oracles C natifs pour les régressions et
les types passent ; à ce jalon l'artefact Methods/WASM disponible localement
était antérieur aux sources et `emcc` semblait manquer. **Aucune parité
numérique WASM n'était alors attestée** pour ces nouveaux profils ; les
résultats du build frais sont consignés dans l'état courant ci-dessus.

### Niveaux de portabilité demandés et priorité de cette livraison

| Niveau | Contrat visé | Statut / gate nécessaire |
|---|---|---|
| 1 — recette native | Le même JSON/YAML désigne des opérations n4m et des primitives DAG-ML/Data par identifiants sémantiques ; chaque hôte les résout vers son binding, avec refus explicite des nœuds inconnus. | **Partiel.** R importe/exporte les profils n4m bornés : SNV/SG/PLS, plusieurs prétraitements dont MSC/EMSC, branches `branch`→`merge: features`, variantes `_or_`/`_cartesian_`, `n4m.SparsePLSDA`, quatorze régressions affines dont N-PLS et MB-PLS et les 25 sélecteurs natifs via `n4m.Selector`. Ceux-ci ont un oracle indépendant R↔Python sur indices/projection et recettes JSON/YAML. Un vrai binaire WASM valide PLS et MB-PLS hors entraînement, pas encore toutes les combinaisons du catalogue. Les alias `r.*` restent propres à R, sans traduction sklearn. |
| 2 — état natif entraîné | Une archive de pipeline n4m/DAG-ML restitue son état pour PREDICT et conserve une recette/lineage permettant un nouveau FIT dans un autre hôte. | **Partiel.** R↔Python échange une enveloppe n4m bornée : v1 PLS, v2 sparse PLS-DA, v3 PLS+SPA, v4 PLS+sélecteurs natifs et v5 quatorze régressions affines, avec N4MM, états MSC/EMSC/sélection, prédiction et nouveau fit sur les profils qualifiés. Les 25 sélecteurs passent l'export/import local ; compositions v4/v5 passent dans les deux sens entre R et Python, y compris MB-PLS à blocs déclarés. Le descripteur affine n'atteste pas méthode ni hyperparamètres : v5 contient une recette déclarée, non une preuve de provenance. RDS et sidecars DAG ne sont pas interlangages. L'ABI C DAG-ML peut assembler un package signé V2/V3, mais le produit R doit encore raccorder l'exécution, le transport Methods et la recharge ; identités, folds, OOF et lineage restent hors de l'enveloppe n4m. |
...
| `n4m` R 1.0.21.9005 (Methods ABI 2.10, `main` `15e89789`) | Publié sur R-universe ; source/Linux/macOS/Windows/WASM `OK` | PLS et variantes, sélection, diagnostics, formule/S3, conversion affine N4MM et 15 opérateurs de prétraitement avec état N4MP natif. | Un accès bas niveau ne qualifie pas chaque méthode en recette, CV et replay ; les états R arbitraires et le catalogue complet ne sont pas portables. |
...
| `nirs4all` R dans `nirs4all-r` | 0.4.0.9031 publié sur R-universe depuis `main` `b716e00` ; source/Linux/macOS/Windows/WASM `OK` | Régression/classification et contrôleurs R optionnels, CV/OOF/refit DAG-ML, recettes JSON/YAML, enveloppes entraînées bornées N4MM v1–v5 et N4MP+N4MM v6 pour les chaînes linéaires qualifiées. | Python #149 est fusionnée côté source, mais cela ne publie pas n4m sur PyPI. Pas d'Archive V2/V3 DAG-ML interlangage générale ni de graphe arbitraire ; les modèles ML/DL R restent des sidecars RDS. |
...
dispatchés : 37 branches de fit/résultat (dont trois AOM/POP qui ne sont pas
des prédicteurs X→y génériques), 25 sélecteurs de variables et deux
diagnostics. Ce compte ne désigne ni 64 prédicteurs sur données inédites ni
64 recettes portables. Le commentaire et la documentation R du dispatcher ont
été corrigés localement pour annoncer 37/25/2 ; les tests du candidat R
parcourent les 25 sélecteurs.
...
Parmi les 37 branches de fit/résultat, quinze ont déjà un contrôleur prédictif
`nirs4all` R dans les candidats locaux (quatorze régressions affines, y compris
N-PLS et MB-PLS, et sparse PLS-DA). Sept autres
...
dispatcher seul, un prédicteur générique X→y réutilisable : état incomplet,
transfert X→X ou rôle AOM/POP distinct. En particulier, `fused_sparse_pls`
...
N-PLS et MB-PLS sont les ajouts du lot courant à cette catégorie. Pour N-PLS, `mode_j` et
...
portables R et Core/WASM ne qualifient pour l'instant qu'une cible. Pour MB-PLS,
les tailles de blocs font partie de la recette et le shim WASM utilise le
véritable intercept affine. Les quinze
...
26 septembre, R-universe sert **0.4.0.9031** depuis `b716e00` et `n4m`
**1.0.21.9005** depuis `15e89789`. Leurs runs publics ont réussi sur
source, Linux, macOS, Windows et WASM ; cela reste une preuve de distribution
R-universe, pas une certification CRAN des tarballs locaux. Le run
...
un avertissement incoming et le workbench local actualisé de 116 sources
n'est qu'un kit de reproductibilité, non un package CRAN à soumettre.
