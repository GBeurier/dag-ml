...
| `nirs4allformats` R 0.2.7 | Avancé | Lecture native, probe, records, dataset, parcours, bytes et sidecars. `nirs4all-r` convertit désormais son dataset homogène en matrice/target/IDs/axe pour fit local et campagne DAG native. | Ce chemin passe par un sidecar RDS et un adaptateur process, pas encore par des data bindings/provider DAG-ML natifs ; le dataset plat n'expose pas toute la provenance des records. |
...
| Intégration `nirs4all-formats` → `nirs4all-r` (`1cb5b20`) | Fixture CSV spectrale décodée par le binding R Rust : 12 échantillons, 8 longueurs d'onde, cible numérique, IDs uniques. Fit/prédiction locaux et FIT_CV→OOF→REFIT→replay DAG natifs passent ; altération d'unité d'axe et IDs dupliqués refusées. Sur la fixture amont réelle de 50×200, le fit local SNV/PLS passe. Contrôle strict `R CMD check --as-cran --no-manual` : 0 erreur, 1 warning incoming. | Le dataset plat `nirs4allformats` n'embarque pas la provenance complète par source ; axes hétérogènes, cubes et chargement multi-fichiers restent hors du convertisseur. Pas encore de data bindings DAG natifs. |
