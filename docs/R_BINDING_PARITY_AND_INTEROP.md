...
| `nirs4all` R 0.4.0.9007 dans `nirs4all-r` | Produit R en développement | Contrôleurs PLS, ridge n4m, `lm`, `ranger`, `glmnet`, `parsnip` régression, `torch` CPU ; matrice et formats ; CV/OOF/refit mono-nœud via DAG-ML ; recettes JSON/YAML et transfert N4MM format 1 PLS seul / format 2 SNV→SG→SIMPLS, corpus d'exécution partagé et accesseurs vers les paquets amont. | Pas encore d'Archive V2/V3 interlangage, de tous les nœuds n4m, de graphe DAG arbitraire ni de runtime R-universe complet. |
...
neuf contrôles verts ; le paquet R `0.4.0.9007` a passé localement les tests
