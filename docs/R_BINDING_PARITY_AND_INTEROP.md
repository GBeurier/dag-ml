...
|---|---|---|
| 1 — recette native | Le même JSON/YAML désigne des opérations n4m et des primitives DAG-ML/Data par identifiants sémantiques ; chaque hôte les résout vers son binding, avec refus explicite des nœuds inconnus. | **Partiel.** Les quatre exemples KS/SNV/SG/PLS passent dans le lecteur R. `nirs4all-r` 0.4.0.9006 exporte JSON/YAML pour SNV/SG/PLS avec les identifiants `n4m.*` et reconstruit un pipeline R depuis cette recette ; Core #15, fusionné après neuf contrôles verts, reconnaît ces alias dans les parseurs Python/Rust/JS/MATLAB. L'ensemble du catalogue n4m, les générateurs comme `_cartesian_` et les graphes DAG arbitraires ne passent pas encore. |
| 2 — état natif entraîné | Une archive de pipeline n4m/DAG-ML restitue son état pour PREDICT et conserve une recette/lineage permettant un nouveau FIT dans un autre hôte. | **Partiel, profil natif exact.** `n4m` R 1.0.21.9002 produit/inspecte N4MM format 2 avec SNV→SG embarqué ; `nirs4all` R 0.4.0.9006 exporte/import l'état avec une recette JSON/YAML séparée et vérifie solveur, composantes, largeur et prétraitement. Un test en processus Python distinct prouve R→Python et Python→R sur spectres bruts à `1e-10`. Le RDS et les sidecars DAG ne sont pas interlangages. Il reste à raccorder l'Archive V2/V3 validée par Rust, identités/manifestes/lineage et recette de réentraînement, puis à élargir les profils/familles. |
...
| `nirs4all` R 0.4.0.9006 dans `nirs4all-r` | Produit R en développement | Contrôleurs PLS, ridge n4m, `lm`, `ranger`, `glmnet`, `parsnip` régression, `torch` CPU ; matrice et formats ; CV/OOF/refit mono-nœud via DAG-ML ; recettes JSON/YAML et transfert N4MM format 2 du profil SNV→SG→SIMPLS, corpus d'exécution partagé et accesseurs vers les paquets amont. | Pas encore d'Archive V2/V3 interlangage, de tous les nœuds n4m, de graphe DAG arbitraire ni de runtime R-universe complet. |
...
des paramètres internes au nœud modèle : les graphes à branches, HPO adaptatif et
...
| `parsnip` régression (`nirs4all` R 0.4.0.9006) | Spécification moteur R conservée dans un sidecar RDS avec empreinte SHA-256 validée par l'adaptateur processus ; fit/prédiction, RDS, CV/OOF/refit/rejeu DAG-ML et sélection inter-familles passent sur Linux avec `parsnip` 1.6.0 (`lm`). `R CMD check --as-cran --no-manual` : 0 erreur, 1 warning incoming, tous les tests et `Suggests` présents. | Seule la régression numérique mono-cible est exposée ; ce sidecar reste R-spécifique et ne rend pas les modèles `parsnip` portables. La couverture multi-moteurs et multi-OS est à faire. |
...
neuf contrôles verts ; le paquet R `0.4.0.9006` a passé localement les tests
