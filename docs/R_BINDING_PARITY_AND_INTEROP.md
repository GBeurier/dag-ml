...
| `nirs4all` R 0.4.0.9011 dans `nirs4all-r` | Produit R en développement | Contrôleurs PLS, ridge n4m, `lm`, `ranger`, `glmnet`, `parsnip` et `mlr3` régression, `torch` CPU ; matrice et formats ; CV/OOF/refit mono-nœud via DAG-ML, y compris groupes explicites sans fuite ; recettes JSON/YAML avec huit familles de prétraitement n4m et expansion `_or_`/`_cartesian_` bornée, transfert N4MM format 1 PLS seul / format 2 SNV→SG→SIMPLS, corpus d'exécution partagé et accesseurs vers les paquets amont. | Pas encore d'Archive V2/V3 interlangage, de tous les nœuds n4m, de graphe DAG arbitraire ni de runtime R-universe complet. Seul SNV/SG/PLS est qualifié pour l'export de recette interlangage ; `mlr3` a été testé sur `regr.rpart` seulement. |
...
neuf contrôles verts ; le paquet R `0.4.0.9011` a passé localement les tests
