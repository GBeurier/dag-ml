...
| `nirs4all` R 0.4.0.9012 dans `nirs4all-r` | Produit R en développement | Contrôleurs PLS, ridge n4m, `lm`, `ranger`, `glmnet`, `parsnip` et `mlr3` régression, `torch` CPU MLP et module régression personnalisé ; matrice et formats ; CV/OOF/refit mono-nœud via DAG-ML, y compris groupes explicites sans fuite ; recettes JSON/YAML avec huit familles de prétraitement n4m et expansion `_or_`/`_cartesian_` bornée, transfert N4MM format 1 PLS seul / format 2 SNV→SG→SIMPLS, corpus d'exécution partagé et accesseurs vers les paquets amont. | Pas encore d'Archive V2/V3 interlangage, de tous les nœuds n4m, de graphe DAG arbitraire ni de runtime R-universe complet. Seul SNV/SG/PLS est qualifié pour l'export de recette interlangage ; `mlr3` a été testé sur `regr.rpart` seulement. Le module torch reste R-spécifique. |
...
| `torch` R | Contrôleur CPU de régression à MLP fixe et module `nn_module` fourni par l'utilisateur, avec fit/prédiction, RDS/fresh-process et CV/OOF/refit/replay DAG-ML testés. | Étendre au GPU, autres pertes/tâches et architectures ; le builder et les poids restent R-spécifiques, ONNX/Python→R séparés. |
