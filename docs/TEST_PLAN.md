...
| Runtime | sequential DAG order, campaign variant x fold execution, data-provider-required paths, fold train/validation data view routing, `NodeTask.data_views`, validation prediction sample checks, `requires_oof` edge enforcement with missing/train/misaligned refusal, refit artifact-handle capture, replay materializes predict views and refit artifact handles, external controller result conformance |
