...
| Data binding | validates envelope fingerprints, feature-set ids, refuses mismatches, checks coordinator relations against campaign folds/leakage policy, idempotently registers the same envelope for multiple replay requirements, materializes in-memory handles and creates scoped data views |
