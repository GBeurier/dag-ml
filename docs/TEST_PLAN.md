...
| Runtime | sequential DAG order, campaign variant x fold execution, data-provider-required paths, fold-aware data view routing, `NodeTask.data_views`, replay materializes predict views and refit artifact handles, external controller result conformance |
