...
%   This wrapper supports one trial at a time. Use the CLI directly with
%   isolated adapter processes for parallel trials.
...
if ~isnumeric(options.parallelTrials) || ~isscalar(options.parallelTrials) || ...
    ...
    error('dagml:HostHpo:ParallelTrials', ...
        ['MATLAB/Octave host HPO currently supports parallelTrials=1 only; ' ...
         'use the CLI directly with isolated adapter workers for parallel trials.']);
end
