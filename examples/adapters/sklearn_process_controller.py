...
def require_prediction_inputs(task: dict[str, Any]) -> None:
    ...
    for key, spec in prediction_inputs.items():
        ...
        # `:refit` / `:predict`-suffixed key with a non-validation partition (test /
        # final), carrying values for the meta-model to predict from — a SEPARATE
        # input from the Validation-OOF meta-features (the leakage invariant).
