...
const pkgDir = path.resolve(process.argv[2] || path.join(repo, "target", "wasm-web", "dag-ml-wasm"));
const dagMl = await import(pathToFileURL(path.join(pkgDir, "dag_ml_wasm.js")).href);
...
const wasmBytes = fs.readFileSync(path.join(pkgDir, "dag_ml_wasm_bg.wasm"));
dagMl.initSync({ module: wasmBytes });
