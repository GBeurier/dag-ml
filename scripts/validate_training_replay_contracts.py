...
OUTCOME_V3_SCHEMA_ID = (
    "https://github.com/GBeurier/dag-ml/schemas/training_replay_outcome.v3.schema.json"
)
...
# Derived from the current base pack by generate_fixtures.py.  This validator
# rejects replay output unless it is bound to exactly these generated bytes.
BASE_PACK_SHA256 = "a9797ece2e63f2162d9c63daa7bf49cc3d299272a4182c8d6379d33ff8ea3584"
BASE_PACK_CHECKSUM = "755e975272de2b18c2ce65a62e5dc56e914066b9d49d6a57db7b45b096fa8045"
...
def validate_targetless_predict_replay_outcome_v3() -> None:
    """Prove the V3 PREDICT fixture differs only by target-free evidence."""

    labeled = _load("training_replay_outcome_predict.v1.json")
    targetless = _load("training_replay_outcome_predict_unlabeled.v3.json")
    require_exact_keys(
        targetless,
        set(labeled),
        set(),
        "training-replay ReplayOutcome.PREDICT-unlabeled",
    )
    validate_strict_json_value(
        targetless, "training-replay ReplayOutcome.PREDICT-unlabeled"
    )
    require(
        targetless["schema_version"] == 3
        and targetless["phase"] == "PREDICT"
        and bool(targetless["outputs"])
        and not targetless["explanations"],
        "target-free replay fixture must be a PREDICT outcome with point outputs only",
    )
    expected = copy.deepcopy(labeled)
    expected["schema_version"] = 3
    for index, identity in enumerate(expected["input_data_identities"]):
        identity["target_content_fingerprint"] = None
        identity["identity_fingerprint"] = w10_fingerprint_without(
            identity, "identity_fingerprint"
        )
        require(
            targetless["input_data_identities"][index]
            == identity,
            f"target-free replay identity[{index}] is not the canonical null-target attestation",
        )
    ...
    require(
        targetless == expected,
        "target-free replay fixture changes evidence beyond schema/target identities",
    )
...
def validate_schema_contracts(
    ...
    outcome_v3_schema = _schema(schemas, OUTCOME_V3_SCHEMA_ID)
    require(
    ...
    require(
        outcome_v3_schema.get("additionalProperties") is False
        and outcome_v3_schema.get("properties", {})
        .get("schema_version", {})
        .get("const")
        == 3,
...
def validate_positive_fixtures() -> None:
    ...
    validate_targetless_predict_replay_outcome_v3()
...
def validate_pack() -> None:
    ...
    require(
        ...
        and len(base_pack.get("artifacts", [])) == 103,
    ...
    require(
        required_runtime_sources <= set(base_entries),
        "pinned D1-D3 base pack omits conformal runtime authority",
    )
